...
// CCC : binning needs to be changed here - the largest bin is 61(NEOS) so, isn't it 100 is enough?	   
   ...
   for(Int_t i=0;i<newBin_NEOS;i++){
   (*fVec)[newBin_DCDYB + newBin_DCDYB + newBin_RENO + i]   = predNEOS->GetBinContent(i+1) ;//predNEOS oscillated?
   tempVec[3] -> SetBinContent(i+1, (*fVec)[newBin_DCDYB + newBin_DCDYB + newBin_RENO + i]);//predNEOS
   (*fData)[newBin_DCDYB + newBin_DCDYB + newBin_RENO + i] = dataNEOS->GetBinContent(i+1) ;//predData
   tempDat[3] -> SetBinContent(i+1, (*fData)[newBin_DCDYB + newBin_DCDYB + newBin_RENO + i]);//predData
   }
   ...
   double scaling4 = tempDat[3]->Integral() / tempVec[3]->Integral();//june  dataNEOS/predNEOS
   ...
   for(Int_t i=0;i<newBin_NEOS;i++){
      (*fData)[newBin_DCDYB + newBin_DCDYB + newBin_RENO + i] = 0; //dataNEOS->GetBinContent(i+1) - (*fVec)[3 * nBins + i] * scaling4; //dataNEOS initialization
   }
// CCC : should be newBin_PROS - Edited
   ...
   if (singleExp.Contains("NEOS")){
	...
	// CCC : should be newBin_DCDYB + newBin_DCDYB + newBin_RENO, check following three lines carefully! - Edited
        ...
        //predNEOS*scaling4(dataNEOS/predNEOS)
...
std::vector<TH1D*> Sterile:: preparePrediction(RooListProxy* _pulls, bool Iosc) const
{
  ...
  for(Int_t i=0;i<newBin_NEOS;i++){//need
    ...
    {
    	forFittingFunction->SetBinContent(i+1, predNEOS->GetBinContent(i+1) );//oscSpectrum-30bin-3.root
    ...
    for(Int_t i=0;i<x100bin_number;i++)
    {
          ...
          forOscFittingFunction2->SetBinContent(i+1, survival_prob_fit * number_of_neutrino[i+1] );//oscSpectrum-2500bin-4.root
    ...
    {
          //w++;
          SUM100 = SUM100 + forOscFittingFunction2->GetBinContent(i+1);
          ...
          predNEOS->SetBinContent( v+13, SUM100/100. );//oscSpectrum-30bin-4.root
    ...
    //predNEOS->SetBinContent(i+1, ((1/1.27702147998)*forOscFittingFunction2_100->GetBinContent(i+1) ));/// 180. );
...
std::vector<TH1D*> Sterile:: prepareData(std::vector<TH1D*> tempPredList) const// 
{
  ...
  */
  ...
  for(Int_t i=0;i<predNEOS->GetNbinsX();i++)//XXX
  {
    ...
    //std::cout<<"1 i : "<<i<<"predNEOS->GetBinContent(i+1) * gradataNEOS->Eval(predNEOS->GetBinCenter(i+1)) : "<<predNEOS->GetBinContent(i+1) * gradataNEOS->Eval(predNEOS->GetBinCenter(i+1))<<std::endl;
  }
  double dataNEOS_Integral = dataNEOS->Integral();
  std::cout<<"dataNEOS->Integral() : "<<dataNEOS->Integral()<<std::endl; //843937
  std::cout<<"Count_NEOSdata : "<<Count_NEOSdata<<std::endl; //355680
  std::cout<<"Count_NEOSdata / dataNEOS->Integral() : "<<( Count_NEOSdata / dataNEOS_Integral )<<std::endl;//0.421453
  
  for(Int_t i=0;i<predNEOS->GetNbinsX();i++)//XXX
  {
    dataNEOS -> SetBinContent(i+1, ( Count_NEOSdata / dataNEOS_Integral ) * predNEOS->GetBinContent(i+1) * gradataNEOS->Eval(predNEOS->GetBinCenter(i+1)) );
    //std::cout<<"2 i : "<<i<<"predNEOS->GetBinContent(i+1) * gradataNEOS->Eval(predNEOS->GetBinCenter(i+1)) : "<<predNEOS->GetBinContent(i+1) * gradataNEOS->Eval(predNEOS->GetBinCenter(i+1))<<std::endl;
  }
  std::cout<<"dataNEOS->Integral() 2 : "<<dataNEOS->Integral()<<std::endl;
